...
在函数外面写try...except...真的是看得人头疼，所以外面尝试定义一个函数，帮助我们完成try...except...，但是这个函数又要能够跟生成器一样暂停和接收值，所以这个函数注定是一个生成器函数。

下面实现了一个例子，我们把add()叫做子生成器，把grouper()叫做委派生成器：
...
def grouper(result):
    ...
    while True:
        x = yield	# yield必不可少
        try:
            _a.send(x)	# 我们只捕获子生成器的StopIteration
        except StopIteration as e:
            res = e.value
            result.append(res)
            # return res        # 为什么不返回？
...
def middle():		# 中间的委派生成器一定要返回值
    res = yield from add()
    return {'result': res}	# return

def grouper(result):	# 接收一个可变对象用于保存返回值
    while True:		# 末层的委派生成器使用while True循环
