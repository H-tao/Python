...
委派生成器相当于管道，可以把任意数量个委派生成器连接在一起：一个委派生成器使用 yield from 调用一个字生成器，而那个子生成器本身也是委派生成器，使用 yield from 调用另一个子生成器，以此类推。最终，这个链条要以一个只使用  yield 表达式的简单生成器结束。

### 8. asyncio

yield from 示例几乎都使用 asyncio 模块做异步编程，因此要有有效的事件循环才能运行。
...
async/await 是用是python3.5后出来的协程异步编程的API, 是为了区分yield，yield from生成器，而使语义更加明确。 
